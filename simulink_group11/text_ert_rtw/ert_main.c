...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(text_M) == (NULL));
  while (runModel) {
    mw_osSignalEventWaitEver(&baserateTaskSem);
    text_step();

    /* Get model outputs here */
    stopRequested = !((rtmGetErrorStatus(text_M) == (NULL)));
  }
...
{
  UNUSED(sig);
  rtmSetErrorStatus(text_M, "stopping the model");
  runModel = 0;
}
