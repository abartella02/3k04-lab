...
#include "Pacemaker_group11_private.h"
#include "rtwtypes.h"
#include "limits.h"
#include "board.h"
...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(Pacemaker_group11_M) == (NULL));
  while (runModel) {
    mw_osSignalEventWaitEver(&baserateTaskSem);
    Pacemaker_group11_step();

    /* Get model outputs here */
    stopRequested = !((rtmGetErrorStatus(Pacemaker_group11_M) == (NULL)));
  }
...
{
  UNUSED(sig);
  rtmSetErrorStatus(Pacemaker_group11_M, "stopping the model");
  runModel = 0;
}
...
int main(int argc, char **argv)
{
  ...
  hardware_init();
  rtmSetErrorStatus(Pacemaker_group11_M, 0);

  /* Initialize model */
  Pacemaker_group11_initialize();
